...
- Name: regression\critical-flows.spec.ts >> Regression - Critical Flows >> FASE 7: ADMIN gets privileges immediately after login >> ADMIN sees admin menu immediately after login without refresh
- Location: tests\regression\critical-flows.spec.ts:30:9
...
Locator: locator('text=Administração')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=Administração')
